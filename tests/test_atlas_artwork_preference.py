...
class AtlasArtworkPreferenceTest(unittest.TestCase):
    def test_setting_defaults_to_stamps_and_renders_cutout_cards(self):
        ...
        self.assertIn("function atlasFamilyMarkup(species, cardHtml)", source)
        self.assertIn("grid.innerHTML = atlasFamilyMarkup(species, cardHtml);", source)
        self.assertIn("atlas-image-card", source)
        self.assertEqual(source.count(".stamp-card[data-sci], .atlas-image-card[data-sci]"), 2)
        self.assertIn("var priorRects = atlasRects(grid);", source)
        self.assertIn("animateAtlasFlip(grid, priorRects);", source)
        ...
        self.assertIn('.atlas-grid[data-artwork="cutouts"] .atlas-fam-grid', STAMPS_CSS.read_text())
        shell = INDEX.read_text()
        self.assertIn('./styles.css?v=r168', shell)
        self.assertIn('./stamps.css?v=r359', shell)
        self.assertIn('./apt.js?v=r185', shell)
